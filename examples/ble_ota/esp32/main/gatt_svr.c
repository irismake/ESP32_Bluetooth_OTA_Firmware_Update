...
static void update_ota_control(uint16_t conn_handle) {
    ...
    case SVR_CHR_OTA_CONTROL_DONE:
        ...
        if (err == ESP_OK) {
            ...
            ESP_LOGE(LOG_TAG_GATT_SVR, "esp_ota_set_boot_partition: %s...", update_partition->label);
           
            if (err != ESP_OK) {
                ESP_LOGE(LOG_TAG_GATT_SVR, "esp_ota_set_boot_partition failed! err=%d", err);
                return;
            }
            ESP_LOGI(LOG_TAG_GATT_SVR, "Done!");
            esp_restart();
