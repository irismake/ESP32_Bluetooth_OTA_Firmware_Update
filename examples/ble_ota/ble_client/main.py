...
async def calculate_checksum(file_path, algorithm='md5'):
    """
    파일의 체크섬을 계산하는 함수
    :param file_path: 체크섬을 계산할 파일 경로
    :param algorithm: 사용할 해시 알고리즘 (기본값: 'md5')
    :return: 파일의 체크섬 값 (16진수 문자열)
    """
    hasher = hashlib.new(algorithm)
    with open(file_path, 'rb') as file:
        while chunk := file.read(65536):  # 64k 크기로 읽기
            hasher.update(chunk)
...
async def send_ota(file_path):
    ...
    async with BleakClient(esp32) as client:
        ...
        file_checksum = await calculate_checksum(file_path)
        print(f"파일의 체크섬 값: {file_checksum}")
