...
#define NEW_DEVICE_NAME "MyESP32"
...
void app_main(void) {
    ...
    if (esp_ota_get_state_partition(partition, &ota_state) == ESP_OK) {
        if (ota_state == ESP_OTA_IMG_PENDING_VERIFY) {
            ...
            ESP_LOGI(LOG_TAG_MAIN, "No OTA update has been detected.");
        }
    
    }
    else if(esp_ota_get_state_partition(partition, &ota_state) == ESP_ERR_INVALID_ARG){
        ESP_LOGI(LOG_TAG_MAIN, "The partition or ota_state argument was NULL.");
    }
    else if(esp_ota_get_state_partition(partition, &ota_state) == ESP_ERR_NOT_SUPPORTED){
        ESP_LOGI(LOG_TAG_MAIN, "The partition is not ota.");
    }
    else if(esp_ota_get_state_partition(partition, &ota_state) == ESP_ERR_NOT_FOUND){
        ESP_LOGI(LOG_TAG_MAIN, "There is no typo data in the partition table or the state cannot be found for that partition.");
    }
    else {
        ESP_LOGI(LOG_TAG_MAIN, "nothing");
    }
